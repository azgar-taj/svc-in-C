...
typedef struct change{
    char add_del_mod;
    char filename[250];
    unsigned int curr_hash_value;
    unsigned int prev_hash_value;
    struct change *prev;
    struct change *next;
}change;
...
typedef struct tracked_files {
    char filename[250];
    int hash;
    struct tracked_files *next;
    struct tracked_files *prev;
}tracked_files;
...
char* convert_to_hex(int hashcode);
...
char *svc_commit(void *helper, char *message) {
    ...
    if(helperc->branches->latest_commit==NULL){
        ...
        current_commit->commit_id = convert_to_hex(get_commit_id(current_commit->changes,message));
         printf("Branch name is : %s : %s\n",current_commit->branch_name,current_commit->commit_id);
        return (current_commit->commit_id);
    }
    return NULL;
}
    ...
    while(b_iter!=NULL){
        ...
        while(c_iter!=NULL){
            if(strcmp(commit_id,c_iter->commit_id)==0){
                printf("In gt commit : %s : %s\n",c_iter->branch_name,c_iter->commit_id);
                return c_iter;
            }
            c_iter = c_iter->prev;
        }
...
char **get_prev_commits(void *helper, void *commit, int *n_prev){
    ...
    struct commit* iter = current_commit;
    ...
    *n_prev = n;
    if(n == 0) return NULL;
    int i = 0;
    for(i = 0; i < n; i++){ls_iter--;}
    return ls_iter;
}

void print_commit(void *helper, char *commit_id){
    commit* iter = (commit*)get_commit(helper,commit_id);
    if(iter==NULL) {printf("Invalid commit id"); return;}
    ...
    int n = 0;
    while(tracks!=NULL){n++; tracks = tracks->next;}
    tracks = iter->files;
...
char **list_branches(void *helper, int *n_branches){
    ...
    char** list ;
    char** iter = list;
    br_iter = (branch*)helperc->branches;
    ...
    while(br_iter!=NULL){
        ...
        *iter = (br_iter->branch_name);
        //printf("\nBranch NAME in LIST: %s",*iter);
        iter++;
        count++;
        br_iter = br_iter->down;
    }
